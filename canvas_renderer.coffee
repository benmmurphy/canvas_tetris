class CanvasRenderer
    constructor: (@canvas) ->
        @colors = new Array(7)
        @image = new Image()
        @image.src = CanvasRenderer.BLOCKS_SRC
        @images = new Array(7)
        @colors[Block.I] = "red" 
        @colors[Block.J] = "blue" 
        @colors[Block.L] = "orange" 
        @colors[Block.O] = "yellow" 
        @colors[Block.S] = "#FF00FF" 
        @colors[Block.T] = "cyan" 
        @colors[Block.Z] = "green" 
        @images[Block.I] =  0
        @images[Block.J] =  80
        @images[Block.L] =  60
        @images[Block.O] =  20
    ...
    renderGrid : (playField) ->
        this.renderBorder(playField)
        this.renderGridColumnSeparators(playField)
        this.renderGridRowSeparators(playField)

    renderBorder: (playField) ->
    ...
    renderGridRowSeparators : (playField) ->
        @ctx.strokeStyle = 'lightblue'
        for col in [0...playField.width]
            for row in [0...playField.height - 1]
                @ctx.beginPath()
                @ctx.moveTo(col * @block_size + 1, (row + 1) * @block_size)
                @ctx.lineTo((col + 1) * @block_size - 1, (row + 1) * @block_size)
                @ctx.closePath()
                @ctx.stroke()
                


    renderGridColumnSeparators : (playField) ->
        @ctx.strokeStyle = 'lightblue'
        for row in [0...playField.height]
            for col in [0...playField.width - 1]
                @ctx.beginPath()
                @ctx.moveTo((col + 1) * @block_size, row * @block_size + 1)
                @ctx.lineTo((col + 1) * @block_size, (row + 1) * @block_size - 1)
                @ctx.closePath()
                @ctx.stroke()
    ...
    renderPlayField : (playField) ->
        ...
        playField.iterate (row, col, value) =>
            if (value != PlayField.FREE_BLOCK) 
                this.drawBlock(value, row, col)


        if (playField.current_block?) 
            playField.current_block.iterate (row, col, value) =>
                this.drawBlock(value, row, col)


        if (playField.preview_block? && !playField.preview_block.atSamePosition(playField.current_block))
            @ctx.save()
            @ctx.globalAlpha = 0.2
            playField.preview_block.iterate (row, col, value) =>
                this.drawBlock(value, row, col)
            @ctx.restore()
